...
@router.post("/products/{product_id}/images/upload", response_model=schemas.ProductImageResponse)
async def upload_image(product_id: int, file: UploadFile = File(...), db: Session = Depends(database.get_db)):
    ...
    # The URL will be /api/catalog/static/filename
...
@router.get("/global-discount", response_model=schemas.GlobalDiscountResponse)
...
@router.post("/global-discount", response_model=schemas.GlobalDiscountResponse)
