...
def create_booking(booking: schemas.BookingCreate, db: Session = Depends(database.get_db)):
    ...
    if new_booking.type == "rent":
        ...
        # Admin notification
        print(f"\n[NOTIFICATION] Sending WhatsApp to Admin:")
...
@app.post("/measurements", response_model=schemas.MeasurementResponse, status_code=status.HTTP_201_CREATED)
...
@app.get("/measurements/booking/{booking_id}", response_model=List[schemas.MeasurementResponse])
...
@app.get("/measurements/user/{user_id}", response_model=List[schemas.MeasurementResponse])
...
@app.post("/bookings/razorpay-order", response_model=schemas.RazorpayOrderResponse)
...
@app.post("/bookings/verify-payment")
