...
export default function Signup() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (res.ok) {
                alert("Account created! A verification code has been sent.");
                router.push('/login');
            } else {
                const err = await res.json();
                ...
                <div className="mt-10 sm:mx-auto sm:w-full sm:max-w-md">
                    <div className="bg-white py-10 px-8 shadow-2xl shadow-slate-200/50 rounded-[40px] border border-slate-100">
                        <form className="space-y-6" onSubmit={handleSubmit}>
                            ...
                            <div className="space-y-1">
                                <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1">Email Address (Optional if phone provided)</label>
                                <input type="email" value={email} onChange={e => setEmail(e.target.value)} className="w-full p-4 bg-slate-50 border border-slate-100 rounded-2xl focus:ring-4 focus:ring-indigo-50 outline-none font-bold text-slate-900" placeholder="your@email.com" />
                            </div>

                            <div className="space-y-1">
                                <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1">Phone Number (Optional if email provided)</label>
                                <input type="text" value={phoneNumber} onChange={e => setPhoneNumber(e.target.value)} className="w-full p-4 bg-slate-50 border border-slate-100 rounded-2xl focus:ring-4 focus:ring-indigo-50 outline-none font-bold text-slate-900" placeholder="+1234567890" />
                            </div>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                <div id="googleBtnSignup" className="w-full flex justify-center"></div>
                                <button 
                                    type="button"
